...
import JSB_MD5 from "db://assets/libs/jsmd5/jsb_md5";
...
export class HotOptions {
    ...
    /** 改prompt */
    onShowPrompt: Function | null = null;
    ...
    init(opt: HotOptions) {
        ...
        oops.res.load('project', (err: Error | null, res: any) => {
            ...
            this.assetsMgr.setVerifyCallback((path: string, asset: jsb.ManifestAsset) => {
                ...
                // 资源文件的大小，但此值可能不存在
                var size = asset.size;
                var resMD5 = JSB_MD5(new Uint8Array(native.fileUtils.getDataFromFile(path)));
                if (resMD5 == asset.md5) {
                    return true
                }
                else {
                    ...
                    this.options.onShowPrompt(oops.language.getLangByID("update_tips_update_fileverifyfail"));
